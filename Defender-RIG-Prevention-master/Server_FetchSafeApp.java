...
/******************
 * This class creates a separate thread for network communication to fetch Safe apps
 * from the remote server.
 * ***************************/
public class Server_FetchSafeApp extends AsyncTask {

    Context context;
    Server_FetchSafeApp(Context c)
    {
        this.context=c;
    }
    //Server script URL
    String SERVER_URL="http://www.guardianapp.16mb.com/RetrieveSafe.php";
    ...
    protected void onPostExecute(Object o) {
        ...
        {
            //Show error message
            Toast.makeText(context,"Unable to fetch safe apps data",Toast.LENGTH_LONG).show();
        }
        else {
            ...
            if (lines != 0) {
                ...
                for (int i = 1; i <= lines; i++) {
                    ...
                    DBHelper.AddGlobalSafeApp(pkgName, appName);
            ...
            Toast.makeText(context, "Safe apps data fetched.", Toast.LENGTH_LONG).show();
        }
    }
    ...
    public String getTimeStamp()
    {
        ...
        return sharedPreferences.getString("safe_timestamp","-1");
    }
    ...
    public void updateTimeStamp(String new_timestamp)
    {
        ...
        editor.putString("safe_timestamp",new_timestamp);
        editor.commit();

        Log.d("pref", sharedPreferences.getString("safe_timestamp", "-1"));
    }
